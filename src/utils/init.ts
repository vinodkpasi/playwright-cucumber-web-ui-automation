...
const reportOverWrite: boolean =
  process.env.REPORT_OVERWRITE != null ? process.env.REPORT_OVERWRITE.toLowerCase() === "true" : true;
try {
  if (reportOverWrite) {
    if (fs.existsSync(reportDirectory)) fs.emptyDirSync(reportDirectory);
  } else {
    if (fs.existsSync(reportDirectory))
      fs.renameSync(reportDirectory, `${reportDirectory}-${moment().format("yyyyMMDDmmss")}`);
  }
